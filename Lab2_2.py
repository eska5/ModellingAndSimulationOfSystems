...
for _ in range(5):
    ...
    Ekin = 0.5 * m * (v_before[0]**2 + v_before[1]**2 + v_before[2]**2)
    Epot = m * g * zI
    Etotal = Ekin + Epot
    Ekin_values.append(Ekin)
    Epot_values.append(Epot)
    Etotal_values.append(Etotal)
    
for i in range(5):
    print(f"{impact_points[i][0]:.4f}, {impact_points[i][1]:.4f}, {impact_points[i][2]:.4f}, {times_between_collisions[i]:.4f}, {Ekin_values[i]:.4f}, {Epot_values[i]:.4f}, {Etotal_values[i]:.4f}")
